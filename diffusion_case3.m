...
Lup(1,:)=[1, 0,                 0, i1*exp(i1*delta),   exp(-i1*delta), 0];
Lup(2,:)=[0, 1,                 -i1*exp(i1*delta), 0,    0, exp(-i1*delta)];
Lup(3,:)=[0, i1*exp(-i1*delta),    1,0,                -exp(i1*delta), 0];
Lup(4,:)=[-i1*exp(-i1*delta), 0,     0,1,                 0, exp(i1*delta)];
Lup(5,:)=[exp(i1*delta), 0,     -exp(-i1*delta), 0,      1,0];
Lup(6,:)=[0, exp(i1*delta),     0, exp(-i1*delta),       0,1];
...
hold on 
plot(time_step, sin(angle(xtu(2,:))))
plot(time_step, sin(angle(xtu(3,:))))
plot(time_step, sin(angle(xtu(4,:))))
plot(time_step, sin(angle(xtu(5,:))))
plot(time_step, sin(angle(xtu(6,:))))
hold off
title('x(t) Up');
xlabel('t')
...
subplot(1,3,2);
plot(time_step, sin(angle(xtd(1,:))))
hold on 
plot(time_step, sin(angle(xtd(2,:))))
plot(time_step, sin(angle(xtd(3, :))))
plot(time_step, sin(angle(xtd(4,:))))
plot(time_step, sin(angle(xtd(5, :))))
plot(time_step, sin(angle(xtd(6,:))))
hold off
title('x(t) Down');
xlabel('t')
legend('\theta_1','\psi_1','\theta_2','\psi_2', '\theta_3','\psi_3')
...
subplot(1,3,3);
plot(time_step, sin(angle(xtl(1,:))))
hold on 
plot(time_step, sin(angle(xtl(2,:))))
plot(time_step, sin(angle(xtl(3, :))))
plot(time_step, sin(angle(xtl(4,:))))
plot(time_step, sin(angle(xtl(5, :))))
plot(time_step, sin(angle(xtl(6,:))))
hold off
title('x(t) Up + Down');
xlabel('t')
legend('\theta_1','\psi_1','\theta_2','\psi_2', '\theta_3','\psi_3')
