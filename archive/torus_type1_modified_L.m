...
D1u = diag(diag(L1_up));
A1u = D1u - L1_up; % infered adjacency between edges
D1u_mod = sum(abs(A1u), 2); 
...
D1d = diag(diag(L1_down));
A1d = D1d - L1_down; % infered adjacency between edges
D1d_mod = sum(abs(A1d), 2); 
...
if max_step/n == 6
    ...
    title('\phi_2 of L_{[1,up]}^{M}', 'fontsize', 12);
    colorbar('Ticks',[0,0.25,0.5,0.75, 1],...
             'TickLabels',{'-\pi','-\pi/2','0','\pi/2','\pi'})
    ...
    title('\phi_1 of L_{[1,down]}^{M}', 'fontsize', 12);
    colorbar('Ticks',[0,0.25,0.5,0.75, 1],...
             'TickLabels',{'-\pi','-\pi/2','0','\pi/2','\pi'})     
    ...
    title('\phi_1 of L_{[1]}^{M}', 'fontsize', 12);
    colorbar('Ticks',[0,0.25,0.5,0.75, 1],...
             'TickLabels',{'-\pi','-\pi/2','0','\pi/2','\pi'})     
    ...
    %saveas(gcf, 'plots\torus_type1_modified_L_spectrum.eps', 'epsc');
    saveas(gcf, 'plots\torus_type1_modified_L_eigvec.png');
...
%ylim([-1 1])
%saveas(gcf, 'plots\torus_type1_modified_L_spectrum.eps', 'epsc');
saveas(gcf, 'plots\torus_type1_modified_L_spectrum.png');
